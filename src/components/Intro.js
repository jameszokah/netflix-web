...
import TADUM_SOUND_URL from "../sounds/netflix_TaDum.mp3";
import "./Intro.css";
...
const Intro = () => {
  ...
  const soundNotification = () => {
    const soundData = new Audio(TADUM_SOUND_URL);
    const sound = soundData.play();

    if (sound !== undefined) {
      sound.then(() => {}).catch((error) => console.error(error));
    }
  };
